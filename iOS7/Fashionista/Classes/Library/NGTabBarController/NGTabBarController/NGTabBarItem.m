...
#define kNGDefaultTitleFont                 [UIFont boldSystemFontOfSize:10.f]
...
+ (NGTabBarItem *)itemWithTitle:(NSString *)title {
    return [NGTabBarItem itemWithTitle:title image:nil];
}
...
- (id)initWithFrame:(CGRect)frame {
    if ((self = [super initWithFrame:frame])) {
        ...
        _titleLabel.textAlignment = NSTextAlignmentLeft;// UITextAlignmentCenter;
...
//        CGFloat imageOffset = self.title.length > 0 ? kNGImageOffset : 0.f;
        ...
        CGFloat textTop = floor(self.bounds.size.height - self.titleLabel.frame.size.height);
...
- (void)drawRect:(CGRect)rect {
    ...
    if (self.image != nil) {
        ...
        CGFloat imageOffset = self.title.length > 0 ? self.imageOffset : 0.f;
...
- (UIFont *)titleFont {
    return _titleFont ?: kNGDefaultTitleFont;
}

- (void)setTitleFont:(UIFont *)titleFont {
    if ([titleFont isEqual:_titleFont]) {
        return;
    }
...
- (void)setSelectedImage:(UIImage *)selectedImage {
    if ([selectedImage isEqual:_selectedImage]) {
        return;
    }
...
- (CGFloat)imageOffset {
    return _imageOffset ?: kNGImageOffset;
}

- (void)setImageOffset:(CGFloat)imageOffset {
    if (imageOffset == _imageOffset) {
        return;
    }
